            ...
            elemento.innerHTML = `
                <div class="objetivo-lista-principal">
                    ...
                    <div class="objetivo-lista-acciones">
                        ...
                        ${!obj.completado ? `<button class="objetivo-lista-btn programar-mañana" title="Programar para mañana" data-id="${obj.id}"><i class="bi bi-calendar-plus"></i></button>` : ''}
...
if (formEditar) {
  formEditar.addEventListener('submit', async function(e) {
    ...
    // Obtener campos de programación
    const programadoPara = document.getElementById('editar-programado-para').value || null;
    const fechaProgramada = document.getElementById('editar-fecha-programada').value || null;
...
document.addEventListener('click', async function(e) {
    ...
    // Configurar campos de programación
    document.getElementById('editar-programado-para').value = objetivo.programado_para || '';
    document.getElementById('editar-fecha-programada').value = formatFechaInput(objetivo.fecha_programada) || '';
    
    // Habilitar/deshabilitar fecha específica según la selección
    const programadoPara = document.getElementById('editar-programado-para');
    const fechaProgramada = document.getElementById('editar-fecha-programada');
    
    fechaProgramada.disabled = programadoPara.value !== 'fecha_especifica';
...
function mostrarFraseRepaso() {
    ...
    contenedor.innerHTML = `
        <div class="frase-repaso-card">
            <div class="frase-texto-repaso mb-4 mx-auto" style="font-size: 1.35rem; color: #000000; line-height: 1.6; max-width: 80%;">
                "${frase.texto}"
            </div>
            ...
            ${frase.notas ? `<div class="frase-notas-repaso mt-3 p-3 bg-light rounded border border-secondary mx-auto" style="color: #2c3e50; font-weight: 600; max-width: 85%;">${frase.notas}</div>` : ''}
        </div>
    `;
...
document.addEventListener('click', function(e) {
    if (e.target.closest('#btn-objetivos-programados')) {
        window.location.href = '/objetivos-programados';
    }
});
...
async function programarObjetivoParaMañana(objetivoId) {
    console.log('📅 Programando objetivo para mañana:', objetivoId);
    ...
    try {
        ...
        if (response.ok) {
            ...
            await cargarObjetivos();
            console.log('✅ Objetivos recargados');
        } else {
